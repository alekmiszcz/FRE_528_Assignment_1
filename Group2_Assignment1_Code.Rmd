...
merged_df <- inner_join(data0, data2, by = c("fips", "year", "kfact")) %>% 
  ...
  mutate(Mean_Precipitation = rowMeans(across(mean_p04 : mean_p09))) %>% # Convert Monthly Precipitation to Yearly
  select(-mean_t04 : -mean_p09) %>% # Drop unwanted temperature and percipitation variables
...
```{r}
# Select specific variables for correlation calculation
...
# Create column and row names for correlation analysis table
...
merged_df_win <- merged_df %>% 
  mutate(
    Profit_per_Acre_Win = Winsorize(
      Profit_per_Acre, 
      val = quantile(Profit_per_Acre, probs = c(0.01, 0.9), na.rm = TRUE) # Get 1st and 90th percentiles for outlier handling
      )
  ) %>% 
...
```{r}
# Define the set of variables to be used in our statistics table
...
m0 <- lm(Profit_per_Acre_Win ~ Mean_Temperature : Temp_Dummy + Mean_Precipitation, data = merged_df_win)
m1 <- lm(Profit_per_Acre_Win ~ Mean_Temperature : Temp_Dummy + Mean_Precipitation + Moist, data = merged_df_win)
m2 <- lm(Profit_per_Acre_Win ~ Mean_Temperature : Temp_Dummy + Mean_Precipitation + Moist + Dry, data = merged_df_win)
m3 <- lm(Profit_per_Acre_Win ~ Mean_Temperature : Temp_Dummy + Mean_Precipitation + Moist + Dry + Soil_Erosion, data = merged_df_win)
...
# Produces: residuals vs fitted, QQ, scale-location, residuals vs leverage plots
